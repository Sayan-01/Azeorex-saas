...
const SheetContent = React.forwardRef<React.ElementRef<typeof SheetPrimitive.Content>, SheetContentProps>(({ side = "right", className, showX, children, ...props }, ref) => (
  <SheetPortal>
    <SheetOverlay />
    ...
    >
      {children}
      <SheetPrimitive.Close className="absolute right-4 bottom-5 rounded-sm opacity-70 ring-offset-background transition-opacity hover:opacity-100 focus:outline-none disabled:pointer-events-none data-[state=open]:bg-secondary">
        {showX && (
          <div className="flex items-center justify-center  bg-zinc-950 w-6 h-6 rounded-full md:hidden">
